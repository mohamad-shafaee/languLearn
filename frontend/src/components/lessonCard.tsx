...
const statusStyles = {
  ...
  raw: "bg-blue-50 border-2 border-blue-200",
  archived: "bg-red-50 border-2 border-red-200",
};
  ...
  return (<div className={`flex px-2 w-full rounded-md ${statusStyles[props.item.status]}`}> 
    ...
    <div className="flex flex-col w-[30%] border-1 border-gray-500">
      
      <div className="flex w-full ">
      <div className="flex w-fit text-xs px-4 py-1 my-1 mr-4 border-2 border-gray-300
       rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onEditi(props.item)}>Edit</div>
       {(props.item.status == 'raw') && <div className="flex w-fit text-xs px-4 py-1
        my-1 mr-4 border-2 border-gray-300 rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onPublishi(props.item)}>Publish</div>}
       {(props.item.status == 'published') && <div className="flex w-fit text-xs px-4 py-1
        my-1 mr-4 border-2 border-gray-300 rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onUnPublishi(props.item)}>Un Publish</div>}
    </div> 
    <div className="flex w-full ">
      {(props.item.status !== 'archived') &&(<div className="flex w-fit text-xs px-4 py-1 my-1 mr-4 border-2 border-gray-300
       rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onRemovei(props.item)}>Remove</div>)}
      {(props.item.status == 'archived') &&(<div className="flex w-fit text-xs px-4 py-1 my-1 mr-4 border-2 border-gray-300
       rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onUnRemovei(props.item)}>UnRemove</div>)}
       ...
       hover:bg-gray-50" onClick={() => {}}>Preview</div>
        ...
        <div className="flex w-full ">
      {(props.item.status !== 'archived') && (<div className="flex w-fit text-xs px-4 py-1
       my-1 mr-4 border-2 border-gray-300
       rounded-md bg-gray-100 hover:cursor-pointer
       hover:bg-gray-50" onClick={() => props.onChangeOpen(props.item, !props.item.is_open)}>
       {props.item.is_open ? "Make Premium" : "Make Open"}</div>)}
    </div>
    </div> 
    </div>);
}
